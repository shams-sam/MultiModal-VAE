...
from data.coco import CocoDet, CocoCap
...
def _get_transform_params(dataset, train):
    ...
    elif dataset in ['cocodet', 'cococap']:
        params = {
    ...
    if train and dataset in ['cifar10', 'cifar100',
                             'cococap', 'cocodet',
                             'imagenet', 'miniimagenet', 'svhn']:
        params['crop'] = True
        params['flip'] = True
        params['rotate'] = 15
...
def get_loader(dataset, batch_size, train=True,
    ...
    if dataset == 'amazon':
        ...
        train = 'train' if train else 'val'
        loader = torch.utils.data.DataLoader(
            datasets.VOCSegmentation('{}/VOC'.format(cfg.data_dir),
                                     image_set=train,
                                     download=cfg.download,
                                     transform=transform,
                                     target_transform=target_transform),
